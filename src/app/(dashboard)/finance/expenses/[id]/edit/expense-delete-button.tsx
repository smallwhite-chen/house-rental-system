...
export function ExpenseDeleteButton({ expenseId, deleteAction }: Props) {
  ...
  function handleDelete() {
    setError("");
    start(async () => {
      const res = await deleteAction(expenseId);
      ...
      {open && (
        ...
        >
          <div className="w-full max-w-md rounded-3xl bg-surface p-6 shadow-xl ring-1 ring-outline-variant">
            <h2 className="mb-2 text-xl font-medium text-on-surface">確認刪除支出</h2>
            <p className="text-sm text-on-surface-variant">
              此操作無法復原。刪除後將無法在報表中追溯此筆紀錄。
            </p>
